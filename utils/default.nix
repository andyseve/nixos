...
let
	...
	inherit (lib) filterAttrs hasSuffix mapAttrs mapAttrs' mapAttrsToList nameValuePair removeSuffix;
in {
	...
	extractAttrs = name: attrs:
		mapAttrs (n: v: v.${name}) attrs;
	
	applyAttrsOnArgs = args: attrs:
		mapAttrs (n: v: v args) attrs;
